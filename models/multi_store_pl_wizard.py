...
class MultiStorePLWizard(models.TransientModel):
    ...
    def _generate_excel_report(self):
        ...
        worksheet.set_column(1, len(self.company_ids) + 1, 15)
        ...
        # Write report
        row = self._write_excel_content(worksheet, stores_data, company_names, totals, 
                                       title_format, header_format, section_format,
    ...
    def _generate_pdf_report(self):
        ...
        pdf = self.env['ir.actions.report']._render_qweb_pdf(
            'custom_invoice_template.report_multi_store_pl',
            self.ids,
            data=data
        )[0]
        
        filename = 'Multi_Store_PL_Report_%s_to_%s.pdf' % (
            self.date_from.strftime('%Y%m%d'),
            self.date_to.strftime('%Y%m%d')
        )
    ...
    def _write_excel_content(self, worksheet, stores_data, company_names, totals,
                            title_format, header_format, section_format,
                            text_format, number_format, total_format):
        '''Write all content to Excel worksheet'''
        ...
        self._write_data_row(worksheet, row, 'XII  Profit/(loss) from discontinuing operations',
                           stores_data, company_names, None, totals,
                           text_format, number_format, total_format)
        row += 1
        
        self._write_data_row(worksheet, row, 'XIII Tax expense of discontinuing operations',
                           stores_data, company_names, None, totals,
                           text_format, number_format, total_format)
        row += 1
        
        self._write_data_row(worksheet, row, 'XIV  Profit/(loss) from Discontinuing operations (after tax)(XII - XIII)',
                           stores_data, company_names, None, totals,
                           text_format, number_format, total_format)
        row += 1
        
        self._write_data_row(worksheet, row, 'XV   Profit / (Loss) (XI+XIV)',
                           stores_data, company_names, 'profit_after_tax', totals,
                           section_format, total_format, total_format)
        row += 1
        row += 1
        ...
        worksheet.write(row, 0, 'XVI  Earning per equity share:', section_format)
        row += 1
        
        self._write_data_row(worksheet, row, '     (1) Basic',
                           stores_data, company_names, None, totals,
                           text_format, number_format, total_format)
        row += 1
        
        self._write_data_row(worksheet, row, '     (2) Diluted',
                           stores_data, company_names, None, totals,
                           text_format, number_format, total_format)
        row += 1
